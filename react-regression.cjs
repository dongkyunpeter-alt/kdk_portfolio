...
(async()=>{
  ...
  await page.getByRole('button',{name:/Projects/}).click(); assert.equal(await page.locator('.common-nav-group').getAttribute('class'),'common-nav-group is-open');
  await page.locator('main').click({position:{x:20,y:20}}); assert.equal(await page.locator('.common-nav-group').getAttribute('class'),'common-nav-group');
  const dpadBefore=Number.parseFloat(await page.locator('#mungi').evaluate(node=>node.style.left)); const rightButton=page.getByRole('button',{name:'오른쪽으로'}); const rightBox=await rightButton.boundingBox(); await page.mouse.move(rightBox.x+rightBox.width/2,rightBox.y+rightBox.height/2); await page.mouse.down(); await page.waitForTimeout(320); await page.mouse.up(); const dpadAfter=Number.parseFloat(await page.locator('#mungi').evaluate(node=>node.style.left)); assert.ok(dpadAfter-dpadBefore>=64);
  ...
  await page.getByRole('button',{name:'게임 건너뛰기'}).click(); await page.mouse.move(1050,650); await page.waitForTimeout(450);
  ...
  const followerDistance=await page.locator('.cursor-mongi').evaluate(node=>{const rect=node.getBoundingClientRect();return {x:Math.abs(rect.left-1054),y:Math.abs(rect.top-656),left:rect.left,top:rect.top}}); assert.ok(followerDistance.x<12&&followerDistance.y<12,JSON.stringify(followerDistance));
