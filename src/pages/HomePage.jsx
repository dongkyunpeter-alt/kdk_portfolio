...
function useProfileTilt(cardRef,portraitRef){
  ...
  useEffect(()=>{
    ...
    if(!card||!portrait||!finePointer.matches||reducedMotion)return;
...
function ProfileTetris({active}){
  ...
  useEffect(()=>{
    if(!player)return;
    const syncPlayback=()=>{
      if(reducedMotion){player.pause();player.setFrame(PROFILE_LOTTIE_HOLD_FRAME);return}
...
function StickyProjectCard({project,index,progress}){
  ...
  const blurProgress=useTransform(progress,fadeRange,index===total-1?[0,0]:[0,3],{clamp:true});
  const blurAmount=useSpring(blurProgress,{stiffness:180,damping:30,mass:.28});
  ...
  return <div className="project-sticky-card" data-project-index={index} style={{top:`calc(var(--project-stack-top) + ${index*18}px)`}}>
    <motion.div className="project-card-layer" style={{scale,y,opacity:cardOpacity,filter}}>
...
function ProjectCardContent({project,index}){
  ...
  return (
      ...
      <div className="project-media">{project.thumbnail?<img src={project.thumbnail} loading="lazy" decoding="async" alt={`${project.title} 프로젝트 미리보기`} width="800" height="450"/>:<div className="project-placeholder">PROJECT SLOT</div>}</div>
      ...
      <div className="project-card-actions" aria-label={`${project.title} 관련 링크`}>
        {actions.map(({label,href,external})=>!href?<button key={label} type="button" disabled><span className="magnetic-label">{label==='깃허브'&&<GitHubIcon/>}{label}</span></button>:<a key={label} href={href} target={external?'_blank':undefined} rel={external?'noopener noreferrer':undefined} aria-label={`${project.title} ${label}${external?' (새 탭)':''}`}><span className="magnetic-label">{label==='깃허브'&&<GitHubIcon/>}{label}{external?' ↗':' →'}</span></a>)}
      </div>
      </article>
  );
}
...
function BoneGame({game,setGame,introReady}){
  ...
  useEffect(()=>{
    ...
    const idle=()=>{
      ...
      if(complete||document.hidden||inputs.size||reducedMotion)return;
  ...
  },[complete,setGame,reducedMotion]);
...
export default function HomePage(){
  ...
  return <main id="main">
    ...
    <GamePopup open={gameOpen} onClose={closeGame} game={game} setGame={setGame}/>{complete&&!reducedMotion&&<CursorMongi/>}
  </main>;
}
