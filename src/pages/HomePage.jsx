...
function BoneGame({game,setGame,introReady}){
  ...
  return <><div className={`game-drop-stage${introReady?' is-ready':''}`}><section className="game-shell arcade-shell" aria-labelledby="game-title">
    ...
    <div className="arcade-controls"><span className="speaker" aria-hidden="true"/><div><small>이동</small><div className="dpad" aria-label="몽이 이동 버튼">{[['up','↑','위로'],['left','←','왼쪽으로'],['down','↓','아래로'],['right','→','오른쪽으로']].map(([dir,label,aria])=><button key={dir} type="button" data-move={dir} aria-label={aria} disabled={complete} onPointerDown={event=>startPad(dir,event)} onPointerUp={()=>stopPad(dir)} onPointerCancel={()=>stopPad(dir)} onLostPointerCapture={()=>stopPad(dir)} onClick={event=>{if(event.detail===0)move(dir)}}>{label}</button>)}</div></div><div className="auto-pickup"><small>건너뛰기</small><button id="quick-unlock" type="button" aria-label="게임 건너뛰기" disabled={complete} onClick={quick}>SKIP</button></div><span className="speaker" aria-hidden="true"/></div>
